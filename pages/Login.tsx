...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem('classNestRememberMe');
    if (savedEmail) {
      setEmail(savedEmail);
      setRememberMe(true);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loggedInUser) {
        if (rememberMe) {
          localStorage.setItem('classNestRememberMe', email);
        } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-secondary p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 transition-all duration-500 transform hover:scale-105">
        <h1 className="text-3xl font-poppins font-bold text-center text-primary mb-2">📘 Class Nest</h1>
